il faut créer une vraie page ContractDetail.tsx et ajouter la route /contracts/:id. Dans le mock actuel, les éléments contrat sont encore embarqués dans MemberDetail via la carte “Contrat actif”, le bouton “Voir détails”, le modal de gel et le modal de création, donc la nouvelle page doit surtout extraire et réorganiser cette logique dans une vue dédiée.[ppl-ai-file-upload.s3.amazonaws]​
À créer
src/app/pages/ContractDetail.tsx
une route /contracts/:id
un lien depuis la fiche membre sur chaque contrat : navigate(/contracts/${contract.id})
La base visuelle existe déjà dans le mock actuel : header sticky, layout en deux colonnes, bloc contrat actif, alertes, timeline, modal de gel, et CTA de renouvellement.[ppl-ai-file-upload.s3.amazonaws]​
Structure recommandée
La nouvelle page doit reprendre la grammaire visuelle de MemberDetail avec un header sticky, une colonne principale 70%, une colonne latérale 30%, et les mêmes patterns de cartes blanches, badges de statut et actions rapides.[ppl-ai-file-upload.s3.amazonaws]​
tsx
// src/app/pages/ContractDetail.tsx
...
export function ContractDetail() {
  ...
  const [contract] = useState({ ...MOCK_CONTRACT, id: Number(id ?? 42) });
    ...
    <div className="flex flex-col h-full overflow-hidden bg-[#F5F6FA]">
      <div className="sticky top-0 z-30 bg-white border-b border-[#E0E0E0] shadow-sm px-8 py-4">
        <div className="flex items-center text-sm text-[#6B7280] font-medium">
          ...
          <Link to="/members" className="hover:text-[#4880FF]">Members</Link>
          <ChevronRight className="w-4 h-4 mx-1" />
          <Link to={`/members/${contract.memberId}`} className="hover:text-[#4880FF]">
            {contract.memberName}
          </Link>
          <ChevronRight className="w-4 h-4 mx-1" />
        ...
        <div className="flex items-center justify-between mt-4">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <h1 className="text-2xl font-bold text-[#111827]">{contract.planName}</h1>
          ...
          <div className="w-full lg:w-[30%]">
            <div className="sticky top-[96px] flex flex-col gap-6">
              ...
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
                ...
                <button
                  onClick={() => setShowFreezeModal(true)}
                  className="w-full mb-2 py-3 rounded-xl bg-[#F5F6FA] hover:bg-[#FFF3D6] text-[#6B7280] hover:text-[#FF9066] font-semibold"
                >
                  Freeze contract
                </button>
                <button
                  onClick={() => toast.success("Renew action ready for API wiring")}
                  className="w-full py-3 rounded-xl bg-[#4880FF] text-white hover:bg-[#3b6ee0] font-semibold"
                >
                  Renew contract
                </button>
              </motion.div>
          ...
          >
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-[#F5F6FA] border border-[#E0E0E0] rounded-xl px-4 py-2.5"
                  />
                </div>
              ...
              <div className="flex gap-3 mt-8">
                <button
                  onClick={() => setShowFreezeModal(false)}
                  className="flex-1 py-2.5 bg-[#F5F6FA] text-[#6B7280] font-semibold rounded-xl"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 py-2.5 bg-[#4880FF] text-white font-semibold rounded-xl"
                >
                  Confirm
                </button>
              </div>
...
Route à ajouter
Le prototype actuel utilise déjà react-router, Link, useNavigate et des pages dédiées par route, donc ajouter /contracts/:id est cohérent avec l’architecture existante.[ppl-ai-file-upload.s3.amazonaws]​
tsx
// routes.tsx
...
Ce qu’il faut modifier dans MemberDetail
Dans la zone contrats, le bouton “Voir détails” doit devenir un vrai lien vers la nouvelle page, au lieu de rester seulement local à la fiche membre. La section contrat actuelle contient déjà les bonnes briques à réutiliser : plan, type, dates, options, freeze et renew.[ppl-ai-file-upload.s3.amazonaws]​
tsx
<button
