...
export function SignUp() {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center relative overflow-hidden bg-[#4880FF] py-12">
      <WavyBackground />

      <div className="w-full max-w-[560px] bg-white rounded-[24px] p-12 shadow-xl z-10 mx-4">
        <h1 className="text-[28px] font-bold text-[#202224] text-center mb-2">Create an Account</h1>
        <p className="text-[14px] text-[#606060] text-center mb-10">Create a account to continue</p>
        ...
        <form className="space-y-6">
          ...
          <div className="space-y-2">
            <label className="text-[14px] text-[#202224] font-medium">Username</label>
            <input 
              type="text" 
          ...
          <div className="flex items-center space-x-3 pt-2">
            <div 
              className={`w-5 h-5 rounded-[4px] border flex items-center justify-center cursor-pointer transition-colors ${accept ? 'bg-[#4880FF] border-[#4880FF]' : 'border-[#C4C4C4] bg-white'}`}
              onClick={() => setAccept(!accept)}
            >
              {accept && <Check className="w-3.5 h-3.5 text-white" />}
            </div>
            <span className="text-[14px] text-[#606060] cursor-pointer" onClick={() => setAccept(!accept)}>I accept terms and conditions</span>
          </div>
          ...
          <p className="text-center text-[14px] text-[#606060] pt-4">
            Already have an account? <Link to="/login" className="text-[#4880FF] font-medium hover:underline">Login</Link>
          </p>
        </form>
      </div>
    </div>
